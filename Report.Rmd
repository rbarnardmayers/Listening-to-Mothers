...
tab1 <- as.data.frame(svytable(~ PREG_INT, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2), 
                        PREG_INT = case_when(PREG_INT == 99 ~ "Missing", 
                                             TRUE ~ PREG_INT))
kable(tab1)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREGCONDITIONC1, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),
                        PREGCONDITIONC1 = case_when(PREGCONDITIONC1 == 99 ~ "Missing",
                                             TRUE ~ PREGCONDITIONC1)) %>% rename(HIGH_BP = PREGCONDITIONC1)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREGCONDITIONC2, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREGCONDITIONC2 = case_when(PREGCONDITIONC2 == 99 ~ "Missing", TRUE ~ PREGCONDITIONC2)) %>% rename(DIABETES = PREGCONDITIONC2)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREGCONDITIONC3, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2)) %>% rename(OTHER = PREGCONDITIONC3)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREGCONDITIONC4, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREGCONDITIONC4 = case_when(PREGCONDITIONC4 == 99 ~ "Missing", TRUE ~ PREGCONDITIONC4)) %>% rename(NONE = PREGCONDITIONC4)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC1, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC1 = case_when(PREPREG_MHCONDC1 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC1)) %>% rename(DEPRESSION = PREPREG_MHCONDC1)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC2, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC2 = case_when(PREPREG_MHCONDC2 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC2)) %>% rename(ANXIETY = PREPREG_MHCONDC2)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC3, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC3 = case_when(PREPREG_MHCONDC3 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC3)) %>% rename(SUBSTANCE_USE = PREPREG_MHCONDC3)
```
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC4, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC4 = case_when(PREPREG_MHCONDC4 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC4)) %>% rename(TRAUMA = PREPREG_MHCONDC4)
...
```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC5, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC5 = case_when(PREPREG_MHCONDC5 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC5)) %>% rename(OTHER = PREPREG_MHCONDC5)

kable(tab1)
```

```{r results = 'asis' ,echo = FALSE}
tab1 <- as.data.frame(svytable(~ PREPREG_MHCONDC6, design = des))
tab1 <- tab1 %>% mutate(Freq = round(Freq, 2),PREPREG_MHCONDC6 = case_when(PREPREG_MHCONDC6 == 99 ~ "Missing", TRUE ~ PREPREG_MHCONDC6)) %>% rename(NONE = PREPREG_MHCONDC6)
...
```{r, echo = FALSE}
LTM_final <- LTM_final %>% 
  mutate(PREPREG_WEIGHT = case_when(is.na(PREPREG_WEIGHT_A1) ~ 2.20462 * PREPREG_WEIGHT_B1, 
         !is.na(PREPREG_WEIGHT_A1) ~ PREPREG_WEIGHT_A1))

summary(LTM_final$PREPREG_WEIGHT)
...
summary(LTM_final$HEIGHT)
```

##### BMI 
```{r, echo = FALSE}
summary(LTM_final$PREPREG_WEIGHT *703 / LTM_final$HEIGHT^2)
```
