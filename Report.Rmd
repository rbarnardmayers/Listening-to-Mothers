---
title: 'Listening to Mothers IV: First Full Survey Report Draft Outline with Item
  Contents and Subgroup Analyses Clarity'
...
# ggbarweight(LTM_final, NUMB_BIRTH, wght) + 
#   theme_classic()

print.cat(NUMB_BIRTH) %>% 
  kbl() %>% 
  kable_styling()
```
...
######  High Blood Pressure 
```{r, echo = FALSE}
tab1 <- print.cat(PREGCONDITIONC1)
...
######  Diabetes 
```{r, echo = FALSE}
tab1 <- print.cat(PREGCONDITIONC2)
...
######  Other Condition 
```{r, echo = FALSE}
tab1 <- print.cat(PREGCONDITIONC3)
...
######  None
```{r, echo = FALSE}
tab1 <- print.cat(PREGCONDITIONC4)
...
tab1 <- print.cat(PREPREG_MHCONDC1)
...
##### Anxiety
```{r results = 'asis' ,echo = FALSE}
tab1 <- print.cat(PREPREG_MHCONDC2)
...
##### Substance Use
```{r, echo = FALSE}
tab1 <- print.cat(PREPREG_MHCONDC3)
...
##### Trauma
```{r results = 'asis' ,echo = FALSE}
tab1 <- print.cat(PREPREG_MHCONDC4)
...
##### Other
```{r results = 'asis' ,echo = FALSE}
tab1 <- print.cat(PREPREG_MHCONDC5)
...
##### None
```{r results = 'asis' ,echo = FALSE}
tab1 <- print.cat(PREPREG_MHCONDC6)
...
##### Height (in inches)
```{r, echo = FALSE}
print.cont(HEIGHT) %>% kbl() %>% kable_styling()

```

##### BMI 
```{r, echo = FALSE}
print.cont(BMI) %>% kbl() %>% kable_styling()
```
...
```{r, echo = FALSE}
print.cat(PROVIDER) %>% kbl() %>% kable_styling()
```

##### By Race
```{r, echo = FALSE}
print.2by2(PROVIDER, RACE) %>% 
  kbl() %>% 
  kable_styling()
```
...
-   Proposed exhibit: primary prenatal use of physicians and midwives by race and ethnicity and by private vs Medicaid insurance

##### By Insurance type (note, not sure how to deal with multiple insurances)
```{r, echo = FALSE}
print.2by2(PROVIDER, INSURANCE) %>% 
  kbl() %>% 
  kable_styling()
```

-   Narrative could report on whether there were differences in type of provider by parity and by whether born in US or abroad
##### By Parity
```{r, echo = FALSE}
print.2by2(PROVIDER, PARITY) %>% 
  kbl() %>% 
  kable_styling()
```
...
##### By Birth Country        
```{r, echo = FALSE}
print.2by2(PROVIDER, BIRTHCOUNTRY) %>% 
  kbl() %>% 
  kable_styling()
```
...
##### Provider Choice
```{r, echo = FALSE}
print.cat(PROVIDERCHOICE) %>% kbl() %>% kable_styling()
```

-   Proposed exhibit: breakdown by race and ethnicity and private vs Medicaid insurance
##### By Race
```{r, echo = FALSE}
print.2by2(PROVIDERCHOICE, RACE) %>% 
  kbl() %>% 
  kable_styling()
```

##### By Insurance (same note about multiple insurance) 
```{r, echo = FALSE}
print.2by2(PROVIDERCHOICE, INSURANCE) %>% 
  kbl() %>% 
  kable_styling()
```

-   Narrative could report on whether there were differences in choice by whether born in US or abroad, whether speaks Eng  or something else at home, by disability status (any vs no  types), by metro vs non-metro residence

##### By Birth country, language, and disability
**NOTE** Not sure how to get metro/non-metro
```{r, echo = FALSE}
print.2by2(PROVIDERCHOICE, BIRTHCOUNTRY) %>% 
  kbl() %>% 
  kable_styling()

print.2by2(PROVIDERCHOICE, LANGUAGE) %>% 
  kbl() %>% 
  kable_styling()

print.2by2(PROVIDERCHOICE, DISABLETECH) %>% 
  kbl() %>% 
  kable_styling()

# print.2by2(PROVIDERCHOICE, METRO) %>% 
#   kbl() %>% 
#   kable_styling()

```

-   Whether had doula support during pregnancy/DOULA [x-ref to childbirth and pp doula results]
##### Had any doula support before, during, or after birth
...
-   Proposed exhibit: breakdown by race and ethnicity and private vs Medicaid insurance  
-   Narrative could report on whether there were differences by parity, by mother’s country of birth, by disability status, by metro vs non-metro residence

#####   If yes, how doula supported/DOULA1
```{r, echo = F}
print.cat(DOULAC1, data = subset(LTM_final, DOULA == "Yes")) %>%
  kbl() %>%
  kable_styling()

print.cat(DOULAC2, data = subset(LTM_final, DOULA == "Yes")) %>%
  kbl() %>%
  kable_styling()

print.cat(DOULAC3, data = subset(LTM_final, DOULA == "Yes")) %>%
  kbl() %>%
  kable_styling()
...
-   [appropriate to consolidate doula results across phases of care; propose that this occurs when reporting birth doula experiences; maybe x-ref here]
-   See also re preg care team: mental health help and childbirth education, later in this chapter

#### Prenatal care

Learning about pregnancy and first prenatal visit,
...
##### No prenatal care by race
```{r echo = FALSE}
print.2by2(PRENAT, RACE) %>%
  kbl() %>%
  kable_styling()

```

##### No prenatal care by insurnace 
```{r echo = FALSE}
print.2by2(PRENAT, INSURANCE) %>%
  kbl() %>%
  kable_styling()

```

##### No prenatal care by pregnancy intention
```{r echo = FALSE}
print.2by2(PRENAT, PREG_INT) %>%
  kbl() %>%
  kable_styling()
...
```{r echo = FALSE}
print.cat(FIRSTVISIT) %>%
  kbl() %>%
  kable_styling()

```

-   Proposed exhibit: breakdown by race and ethnicity 

```{r, echo = F} 

print.2by2(FIRSTVISIT, RACE) %>%
  kbl() %>%
  kable_styling()

```

-   Proposed exhibit: breakdown by private vs Medicaid insurance

```{r, echo = F} 

print.2by2(FIRSTVISIT, INSURANCE) %>%
  kbl() %>%
  kable_styling()
...
##### First visit by High Blood Pressure
```{r, echo = F} 

print.2by2(FIRSTVISIT, PREPREG_PHYSCONDC1) %>%
  kbl() %>%
  kable_styling()

```

##### First visit by Diabetes
```{r, echo = F} 

print.2by2(FIRSTVISIT, PREPREG_PHYSCONDC2) %>%
  kbl() %>%
  kable_styling()

```

##### First visit by Other
```{r, echo = F} 

print.2by2(FIRSTVISIT, PREPREG_PHYSCONDC3) %>%
  kbl() %>%
  kable_styling()
...
######   Proposed exhibit: simple bar graph in descending order of reasons with Overall throughline 

```{r echo = FALSE}
# Need the NOPRENATALC1 - NOPRENATALC14
# ggbarweight(subset(LTM_final, FIRSTVISIT == "No"), NOPRENATALC1, wght) +
#   theme_classic()
```
...
```{r}
# CARESETTINGC1 - CARESETTINGC9
```

-  Proposed exhibit: simple bar graph in descending order with overall throughline
#####   Individual and group prenatal care
#####   Individual versus group care/CARETYPE

```{r}
# CARETYPEC1 - CARETYPEC3
```
...
```{r echo = FALSE}
LTM_final$CARETYPE1 <- refac.fun("CARETYPE1", vars = c("I don't know", "I'd prefer not to answer", "Missing"))

print.cat(CARETYPE1) %>%
  kbl() %>%
  kable_styling()
```
-  Proposed exhibit: by race and ethnicity and type of insurance

#####   Preference between individual and group care type (if experienced both)/CARETYPEPREF

``` {r}
print.cat(CARETYPEPREF) %>%
  kbl() %>%
  kable_styling()
```
...
``` {r}
print.2by2(CARETYPEPREF, RACE) %>%
  kbl() %>%
  kable_styling()

print.2by2(CARETYPEPREF, INSURANCE) %>%
  kbl() %>%
  kable_styling()
```
...
```{r}
print.2by2(CARETYPEPREF, PARITY) %>%
  kbl() %>%
  kable_styling()

print.2by2(CARETYPEPREF, EDUCATION) %>%
  kbl() %>%
  kable_styling()

# print.2by2(CARETYPEPREF, AGE) %>%
#   kbl() %>%
#   kable_styling()
```
...
-   Proposed exhibit: simple bar graph in descending order with Overall throughline
-   Whether had equipment/supplies for 4 types of home monitoring/ATHOMECARE
-   Confidence about ability to do four types of home monitoring/BPCONFID + URINECONFID + WEIGHCONFID + BABYHRCONFID

```{r echo =F}
#ATHOMECAREC1 = ATHOMECAREC6
```
...
##### Preference between in-person and televisits (if experienced both)/CAREMODEPREF
```{r}
print.cat(CAREMODEPREF) %>% 
  kbl() %>% 
  kbl_styling()
```

-   Proposed exhibit: by race and ethnicity, metro vs non-metro residence (bars, including overall, showing in-person, televisit, mix) 
- Narrative could report whether preference differed by parity, education, whether employed during pregnancy, private vs Medicaid insurance
